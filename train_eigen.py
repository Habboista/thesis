...
def train(model, optimizer, criterion, train_loader) -> dict[str, list[float]]:
    training_info: dict[str, list[float]] = {'train_loss': [], 'train_reg': []}
    model.train()
    for image, depth_map, valid_mask, overlap_mask in tqdm(train_loader):
        optimizer.zero_grad()
        ...
        # compute loss
        train_loss = criterion(pred[valid_mask], torch.log(depth_map[valid_mask]))
        loss = train_loss + 0.01 * reg
